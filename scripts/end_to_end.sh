...
strings /tmp/test/ls | grep Pestilence > /dev/null && echo "[KO] Pestilence should not infect when 1065a3d95aa5451b is running" && exit 1
...
strings /tmp/test/ls | grep Pestilence > /dev/null && echo "[KO] Pestilence should not infect when ran in GDB" && exit 1
echo "[OK] Pestilence did not infect hosts when ran with GDB"

./Pestilence
strings /tmp/test/ls | grep Pestilence > /dev/null || { echo "[KO] Pestilence should infect when ran alone"; exit 1; }
...
./Pestilence
strings /tmp/test2/ls | grep Pestilence > /dev/null || { echo "[KO] Pestilence should infect binaries in /tmp/test2"; exit 1; }
echo "[OK] Pestilence infects binaries in /tmp/test2"

mv /tmp/test2/ls /tmp
./scripts/copy_hosts.sh > /dev/null
/tmp/ls > /dev/null
strings /tmp/test/ls | grep Pestilence > /dev/null || { echo "[KO] Infected binary should infect other binaries in /tmp/test"; exit 1; }
echo "[OK] Infected binary infects other binaries in /tmp/test"

cp $(which ls) /tmp/test2
chmod +w /tmp/test2/ls
/tmp/ls > /dev/null
strings /tmp/test2/ls | grep Pestilence > /dev/null || { echo "[KO] Infected binary should infect other binaries in /tmp/test2"; exit 1; }
...
echo "[OK] /tmp/test/ls is working with valgrind"
