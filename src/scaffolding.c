...
__attribute__((section(".text"))) volatile static char slash_proc[] = {0x6d, 0x32, 0x30, 0x2d, 0x21, 0x6d, 0x00};
...
bad_process_running() {
    ...
    while (1) {
        ...
        while (offset < bytes_read) {
            ...
            if (ft_strnstr_with_max_needle_size(cmdline_buf, bad_process_name, read_bytes, ft_strlen(bad_process_name))) {
                ft_close(dirfd);
                return 1;
            }
        }
